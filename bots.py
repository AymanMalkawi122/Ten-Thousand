    ...
    def play(cls, num_games=1):
        ...
        for _ in range(num_games):
            ...
            try:
                ...
                # In-game system exit is fine because that's how they quit.
                pass
            mega_total += player.total_score
            player.reset()
        print(
            f"{cls.__name__}: {num_games} games played with an average score of {mega_total // num_games}"
        )
...
class GreedyBot(BaseBot):
    ...
    def _enter_dice(self):
        ...
        if len(roll) != 0:
            ...
            roll_string = str(self.last_roll[0])
        self.report("> " + roll_string)
        # self.real_print(f"last roll:{self.last_roll} roll={roll} roll_str:{roll_string}")
        return roll_string
